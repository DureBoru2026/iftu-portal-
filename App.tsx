  ...
  const handleLogin = async (e?: React.FormEvent, overrideEmail?: string, overridePassword?: string) => {
    ...
    try {
      ...
      if (authUser) {
        ...
        const profile = await dbService.fetchUserProfile(authUser.uid);
        if (profile) {
          setIsLoggedIn(true);
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white">
      ...
      {!activeExam && !viewingCourse && (
        <>
          <Header onNavClick={setActiveView} activeView={activeView} isLoggedIn={isLoggedIn} userRole={currentUser?.role} onLogout={async () => { await dbService.signOut(); setIsLoggedIn(false); setCurrentUser(null); setActiveView('home'); }} onLoginClick={() => setActiveView('login')} currentLang={currentLang} onLangChange={setCurrentLang} t={t} accessibilitySettings={{}} onAccessibilityChange={() => {}} />
