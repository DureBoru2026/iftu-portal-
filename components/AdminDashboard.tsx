...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ 
  ...
  const [activeTab, setActiveTab] = useState<'identities' | 'curriculum' | 'bulletins' | 'analytics' | 'results' | 'backups'>('identities');
  ...
  return (
    <div className="space-y-16 animate-fadeIn pb-32">
      ...
      <div className="flex flex-wrap gap-4 border-b-8 border-black pb-4 no-print">
        {['identities', 'curriculum', 'bulletins', 'analytics', 'results', 'backups'].map((t) => (
          <button 
            key={t} 
      ...
      {activeTab === 'analytics' && (
        <div className="space-y-16 animate-fadeIn printable-transcript" id="analytics-report-container">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
             ...
             <div className="bg-blue-600 text-white border-8 border-black rounded-[4rem] p-10 shadow-[20px_20px_0px_0px_rgba(0,0,0,1)] flex flex-col justify-between">
                <div className="space-y-2">
                   <h4 className="text-2xl font-black uppercase italic">Weekly Pulse</h4>
                   <p className="text-[10px] font-black uppercase tracking-widest opacity-60">vs Previous Week</p>
                </div>
                <div className="py-8 text-center">
                   <p className="text-7xl font-black italic">
                      +{users.filter(u => new Date(u.joinedDate) > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)).length}
                   </p>
                   <p className="text-xs font-black uppercase tracking-widest mt-2">New Citizens</p>
                </div>
             ...
             <div className="flex gap-4">
                <button 
                  onClick={() => {
                    ...
                    const filtered = examResults.filter(r => new Date(r.completedAt) > lastWeek);
                    downloadCSV(filtered, "National_Results_Previous_Week");
                  }} 
                  className="bg-blue-600 text-white px-8 py-3 rounded-2xl border-4 border-black font-black uppercase text-[10px] shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate-y-1"
                >
                  📥 Back Up Previous Week (CSV)
                </button>
                <button onClick={() => downloadCSV(examResults, "National_Exam_Results")} className="bg-green-600 text-white px-8 py-3 rounded-2xl border-4 border-black font-black uppercase text-[10px] shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate-y-1">Export All Results (CSV)</button>
             </div>
          </div>
          <div className="bg-white border-8 border-black rounded-[5rem] overflow-hidden shadow-[30px_30px_0px_0px_rgba(0,0,0,1)]">
             <table className="w-full text-left border-collapse">
      ...
      {/* Backups & Archive View */}
      {activeTab === 'backups' && (
        <div className="max-w-5xl mx-auto space-y-16 animate-fadeIn pb-32">
           <div className="bg-white border-[10px] border-black rounded-[5rem] p-12 md:p-20 shadow-[30px_30px_0px_0px_rgba(0,0,0,1)] relative overflow-hidden">
              <div className="absolute top-0 left-0 w-full h-8 ethiopian-gradient"></div>
              <div className="space-y-10 relative z-10">
                 <h2 className="text-7xl md:text-9xl font-black uppercase italic tracking-tighter leading-none text-blue-900">Sovereign <br/>Archive.</h2>
                 <p className="text-2xl font-black italic max-w-2xl opacity-70">
                    "Ensure national digital education resilience. Export and secure previous week's intellectual property and performance benchmarks."
                 </p>
              </div>
           </div>
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
              ...
              <div className="bg-white border-8 border-black rounded-[4rem] p-12 space-y-10 shadow-[20px_20px_0px_0px_rgba(59,130,246,1)]">
                 <div className="flex justify-between items-center">
                    <h3 className="text-4xl font-black uppercase italic">Previous Week</h3>
                    <span className="text-5xl">📅</span>
                 </div>
                 <div className="space-y-4">
                    <div className="flex justify-between font-black uppercase text-[10px]">
                       <span>Knowledge Points Minted</span>
                       <span className="text-blue-600">
                          {examResults
                            .filter(r => new Date(r.completedAt) > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000))
                            .reduce((sum, r) => sum + r.score, 0)
                            .toLocaleString()} KP
                       </span>
                    </div>
                    <div className="flex justify-between font-black uppercase text-[10px]">
                       <span>New Module Registrations</span>
                       <span className="text-green-600">
                          {users.filter(u => new Date(u.joinedDate) > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)).length} Students
                       </span>
                    </div>
                 </div>
                 <button 
                   onClick={() => {
                     ...
                     const backupData = {
                       ...
                       results: examResults.filter(r => new Date(r.completedAt) > lastWeek),
                       users: users.filter(u => new Date(u.joinedDate) > lastWeek)
                     };
                     const blob = new Blob([JSON.stringify(backupData, null, 2)], { type: 'application/json' });
                     const url = URL.createObjectURL(blob);
                     const link = document.createElement('a');
                     link.href = url;
                     link.download = `Sovereign_Backup_Week_${new Date().toISOString().split('T')[0]}.json`;
                     link.click();
                   }}
                   className="w-full py-8 bg-black text-white rounded-[2.5rem] border-8 border-black font-black uppercase text-xl shadow-[8px_8px_0px_0px_rgba(59,130,246,1)] hover:translate-y-2 transition-all"
                 >
              ...
              {/* Full System Export */}
              <div className="bg-white border-8 border-black rounded-[4rem] p-12 space-y-10 shadow-[20px_20px_0px_0px_rgba(0,155,68,1)]">
                 <div className="flex justify-between items-center">
                    <h3 className="text-4xl font-black uppercase italic">Full Registry</h3>
                    <span className="text-5xl">🏦</span>
                 </div>
                 <p className="text-xs font-black uppercase text-gray-400 italic">Total Identity & Curriculum Archive Package</p>
                 <button 
                   onClick={() => {
                     const fullBackup = {
                       timestamp: new Date().toISOString(),
                       version: '2.0.0',
                       author: 'Jemal Fano Haji',
                       data: { users, courses, exams, news, examResults }
                     };
                     const blob = new Blob([JSON.stringify(fullBackup, null, 2)], { type: 'application/json' });
                     const url = URL.createObjectURL(blob);
                     const link = document.createElement('a');
                     link.href = url;
                     link.download = `IFTU_Full_Sovereign_State_${new Date().toISOString().split('T')[0]}.json`;
                     link.click();
                   }}
                   className="w-full py-8 bg-green-600 text-white rounded-[2.5rem] border-8 border-black font-black uppercase text-xl shadow-[8px_8px_0px_0px_rgba(0,0,0,0.5)] hover:translate-y-2 transition-all"
                 >
                   State-Level Backup →
                 </button>
                 <p className="text-[8px] font-black uppercase text-center opacity-40">National Cloud Sync (Supabase) is Active</p>
              </div>
           </div>
        </div>
      )}
